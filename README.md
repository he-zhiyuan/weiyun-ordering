...
味云点餐是一套面向餐饮门店的外卖点餐系统，覆盖商家管理端和用户点餐端两侧完整业务链路。后端围绕 Spring Boot + MyBatis 构建了分层清晰的多模块工程（`weiyun-common` / `weiyun-pojo` / `weiyun-server`），管理端与用户端分别使用独立的 JWT 密钥体系鉴权，并通过 WebSocket 实现来单 / 催单的实时推送。

除核心业务功能外，还完整实现了一条从代码提交到生产上线的自动化部署链路：GitHub Actions 触发部署 → SSH 远程执行受限脚本 → 前后端分别构建 → systemd 管理后端进程、Nginx 托管前端静态资源，并针对部署密钥做了权限隔离设计（触发部署与拉取代码使用两把用途不同、权限最小化的密钥）。用户端在浏览器点餐场景下的登录、下单、支付流程均为独立设计（详见「已知局限」）。
